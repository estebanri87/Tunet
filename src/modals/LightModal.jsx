...
import { normalizeCustomRows } from './editCard/customRows';
...
export default function LightModal({
  ...
  // Card-defined rows are reason enough to show the right panel, even for a
  // plain on/off light that has no controls of its own.
  const hasCustomRows = normalizeCustomRows(settings?.customRows).length > 0;
  const statusRows = useStatusRows(settings?.customRows, entities, t);
  const showRightPanel = isDimmable || groupedEntityIds.length > 0 || hasCustomRows;
