...
} from '../components/cards/customItems';
import {
  convertValueByKind,
  formatUnitValue,
...
export default function WeatherModal({
  ...
  const modalTitleId = `weather-modal-title-${activeWeatherEntity?.entity_id?.replace(/[^a-zA-Z0-9_-]/g, '-') || 'weather'}`;
  ...
  // Own sensors replace the attribute-derived defaults when configured, since
  ...
  const customDetails = getRenderableCustomItems(settings?.detailItems, entities).map((item) => {
    ...
    return {
      ...
      value: unit ? `${text} ${unit}` : text,
      iconName: item.icon || 'mdi:gauge',
    };
  });
  ...
  const details = customDetails.length > 0 ? customDetails : defaultDetails;
