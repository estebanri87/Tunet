...
function FlowLine({ active, reverse, x1, y1, x2, y2 }) {
  return (
    <line
      ...
      strokeLinecap="round"
      className={active ? `energy-flow-line-active${reverse ? ' energy-flow-line-reverse' : ''}` : ''}
      opacity={active ? 0.9 : 0.35}
    />
  );
}

const EnergyFlowCard = memo(/** @param {any} props */ function EnergyFlowCard({
  cardId,
  gridEntityId,
  solarEntityId,
  batteryEntityId,
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mt-4 flex flex-1 items-center justify-center">
        <svg
          viewBox="0 0 200 120"
          className="h-full w-full max-h-[140px]"
        ...
        >
          ...
          {/* Solar -> Home */}
          {hasSolar && (
            <FlowLine active={flows.solarToHome} reverse={false} x1={30} y1={60} x2={150} y2={60} />
          )}
